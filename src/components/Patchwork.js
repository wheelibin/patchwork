...
class Patchwork extends Component {
  ...
  async componentDidMount() {
    ...
    const patchMarkup = patchId ? await patchworkApi.getPatch(patchId) : patches.basicKrell;
    const patch = patchbook.parse(patchMarkup);
    this.setState({ markup: patchMarkup, patch: patch, displayVoices: patch.voices.map(v => v.name) });
  }
